...
| **Agent** (tag beside the software) | That panel runs Kiosk Satellite in agent mode: no dashboard, voice or screensaver, just the device in Home Assistant with its sensors, the remote admin and updates. Its Display and Dashboard columns read "—". |
